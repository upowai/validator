...
async def sign_and_push_transactions(transactions):
    try:
        ...
        if transactions:
            ...
            validatorTransactionsCollection.delete_many(
...
def process_all_transactions():
    try:
        ...
        all_transactions = list(
            validatorTransactionsCollection.find().sort("timestamp", 1)
        )
...
def add_transaction_to_batch(wallet_address, tokens_to_distribute, rewardType):
    try:
        ...
        validatorTransactionsCollection.insert_one(transaction_document)
