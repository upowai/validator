...
from database.mongodb import validatorBalanceUpdateData


def store_in_db(block_height, updates):
    if validatorBalanceUpdateData is None:
        logging.info("Database collection not initialized.")
        return

    if not isinstance(block_height, str) or not isinstance(updates, dict):
        logging.error("Invalid input types for block_height or validator_updates.")
        return

    if not updates:
        logging.info("Empty Validator updates. Nothing to store.")
        return

    try:
        existing_document = validatorBalanceUpdateData.find_one(
        ...
        validatorBalanceUpdateData.insert_one(document)
...
def retrieve_from_db(block_height):
    if validatorBalanceUpdateData is None:
        ...
        return None
    try:
        document = validatorBalanceUpdateData.find_one({"block_height": block_height})
